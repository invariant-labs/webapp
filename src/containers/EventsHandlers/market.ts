  ...
  useEffect(() => {
    ...
    const connectEvents = () => {
      ...
      if (
        ...
        !currentList.find((addr: string) => addr === lastTokenFrom)
      ) {
        currentList.push(lastTokenFrom)
      }

      if (
        lastTokenTo !== null &&
        !tokens[lastTokenTo] &&
        !currentList.find((addr: string) => addr === lastTokenTo)
      ) {
        currentList.push(lastTokenTo)
      }
