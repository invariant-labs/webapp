...
export const IdoLabelTimerRushHourMode: React.FC<IProps> = ({
  name,
  periodEnd,
  isDarkBg=false,
  ...
  const classes = useStyles()
    ...
    let [rushHour, setRushHour] = useState(false)
...
<h1 style ={{color:"orangered"}}> {day<10 ? `0${day}`: `${day}`}:{hour<10 ? `0${hour}`: `${hour}`}:{minute<10 ? `0${minute}`: `${minute}`}:{second<10 ? `0${second}`: `${second}`}  </h1>:
