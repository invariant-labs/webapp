...
import JupiterIndexingPopover from './JupiterIndexingPopover/JupiterIndexingPopover'
...
export const NewPosition: React.FC<INewPosition> = ({
  ...
  const [cacheJupList, setCacheJupList] = useState<CacheJup[]>([])
  ...
  }, [initialTokenFrom, initialTokenTo, initialFee, cacheJupList])
  ...
  useEffect(() => {
    fetch('https://cache.jup.ag/markets?v=3')
      .then(async response => await response.json())
      .then(data => {
        setCacheJupList(data)
      })
      .catch(error => console.error('Error fetching data:', error))
  }, [])

  useEffect(() => {
    const isAddressPresent = cacheJupList.some((item: CacheJup) => item.pubkey === address)
    setindexingActive(isAddressPresent)
  }, [address])
  ...
  return (
    <>
      <JupiterIndexingPopover open={open} indexingActive={indexingActive} onClose={handleClose} />
      <Grid container className={classes.wrapper} direction='column'>
        ...
        <Grid container className={classes.headerWrapper}>
          ...
          <img
            onClick={handleClick}
            className={indexingActive ? classes.jupiterLogo : classes.jupiterLogoInactive}
            height='32px'
